...
| --- | --- | --- | --- | --- | --- |
| 首次摊位审核 | 摊主提交、系统管理员审核 | 经营者、地点、品类、营业时间、照片等摊位资料 | 核验资料并通过或未通过 | 审核结果；通过后进入发证，未通过则返回理由 | 发放设置证/摊主修改后重新提交 |
| 发证与二维码生成 | 系统管理员 | 已通过的首次摊位审核、证件有效期 | 发放临时摊位设置证并生成唯一二维码 | 设置证、二维码、H5 首个公开版本 | 公众查询 |
| 摊主资料变更 | 摊主提交、系统管理员审核 | 经营资料变更申请 | 原值/新值对比，整单通过或未通过 | 新生效版本或未通过理由 | H5 更新/摊主修改后重新提交 |
| 设置证临期续审 | 系统提醒、摊主提交、系统管理员审核 | 临期设置证、最新摊位资料、续审材料 | H5 提醒，摊主提交，管理员重新审核并更新有效期 | 新设置证及有效期或未通过理由 | H5 证照更新/重新提交 |
...
| 摊主认证申请 | H5 未认证用户提交、系统管理员审核 | 实名信息、联系方式、摊位关系及认证材料 | 通过或审核未通过 | 摊主身份或带理由的未通过结果 | 首次摊位信息提交/修改后重新提交 |
...
| 审核结果 | 审核只保留“审核通过、审核未通过”两种结果 | 明确提及 |
| 审核未通过 | 选择审核未通过必须填写理由；申请保留并返回申请人，修改后可重新提交；首次申请未通过时不发证、不生成二维码 | 明确提及 |
| 摊主认证入口 | 未认证用户的 H5“我的”页提供摊主认证入口 | 明确提及 |
| 后台准入 | 仅系统管理员可登录 PC 后台；摊主、执法者和普通用户访问后台时一律拒绝 | 明确提及 |
| 二维码生成 | 只有首次摊位审核通过并完成设置证发放后才能生成；停用后保留历史记录 | 明确提及 |
| 服务分类 | 分类图标和状态由系统管理员维护；停用分类前需提示关联点位 | 合理推断 |
| 辖区校验 | 地图点位保存或发布时按所属辖区边界校验，越界进入异常处理 | 合理推断 |
| 操作日志 | 关键管理操作必须留痕，日志只读且不可由后台用户删除 | 合理推断 |
...
| --- | --- |
| 摊位 | 正常经营、暂停经营、已注销 |
| 摊位审核 | 草稿、待审核、审核通过、审核未通过、已重新提交 |
| 二维码 | 未生成、已生成、已停用 |
| 临时摊位设置证 | 待发放、有效、临期、续审中、已过期、已作废 |
| H5 公开版本 | 未发布、已发布、更新审核中、已下线 |
| 身份认证 | 未提交、待审核、已认证、审核未通过、已重新提交、已停用 |
| 地图点位 | 启用、停用、待核验 |
